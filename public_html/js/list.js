...
function List() {
    ...
    this.build_lists = function() {
        ...
        for (var i in this.response.tasks) {
            ...
            // build main list
                ...
                if (this.type == 1 && this.response.tasks[i].group) {
        ...
        for (var i in this.expecting) {
            if (this.expecting[i]) {
                ...
                for (var k in this.response.counts[i]) {
                    $('.tabs .c', LISTS[i]).eq(k)
    ...
    this.build_task_json = function(i) {
        ...
        html_task.children('.due').children('.editable')
            ...
            .bind('click', row_handler.replace_html);
        var html_followers = TEMPLATES.followers.clone();
    ...
    this.resize = function() {
        ...
        list_handler.main_per_page = parseInt(
            ((LISTS[0].height() - PIXELS.mainminus
            ) / PIXELS.rowheight)
        );

        // calculate mini tasks per page
        list_handler.mini_per_page = parseInt(
            ((LISTS[1].height() - PIXELS.miniminus
            ) / PIXELS.rowheight)
        );
        ...
        if ($('.table', LISTS[0])
                .height() != list_handler.main_per_page
                * PIXELS.rowheight) {
            $('.table', LISTS[0])
                .height(list_handler.main_per_page
                * PIXELS.rowheight);
        ...
        if (!url_handler.hash_last) reload = true;
        if (reload) {
            list_handler.get_lists();
        }
    }
    ...
    handle_pager_main = function() {
        var page = parseInt(url_handler
            .get_url_param(PARAMS.mainpage, $(this).attr('href')));
        if (list_handler.last_search_q.length > 0) {
            search_page = page;
            list_handler.last_search_q = list_handler.search_val;
            list_handler.do_search();
    ...
    handle_pager_mini = function() {
        var page = parseInt(url_handler
            .get_url_param(PARAMS.minipage, $(this).attr('href')));
    ...
    handle_change_group = function(e) {
        var group = parseInt(url_handler
            .get_url_param(PARAMS.group, $(this).attr('href')));
